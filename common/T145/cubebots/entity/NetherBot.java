...
public class NetherBot extends CubeBot {
	...
	protected void dropFewItems(boolean recentlyAttacked, int lootModify) {
		if (rand.nextInt(7) <= (tamed ? 1 : 5))
			dropItem(new ItemStack(CubeBots.netherPiece, new Random().nextInt(2) + 1));
		if (rand.nextInt(150) == 22)
			dropItem(new ItemStack(CubeBots.cubeBotCore));
		if (rand.nextInt(10) == 0)
			dropItem(new ItemStack(Item.goldNugget, new Random().nextInt(2) + 1));
		if (rand.nextInt(170) == 0)
			dropItem(new ItemStack(Item.swordGold, new Random().nextInt(2) + 1));
	}
